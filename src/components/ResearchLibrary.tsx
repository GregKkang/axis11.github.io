  ...
  return <section id="research" className="bg-cream py-20 md:py-24 scroll-mt-16" aria-labelledby="research-title">
    <div className="container mx-auto max-w-6xl px-6">
      ...
      <div className="grid sm:grid-cols-[2fr_1fr] gap-4 mb-6">
        <label className="block"><span className="block text-xs font-semibold uppercase tracking-wider text-navy mb-2">Search articles</span><span className="relative block"><Search className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground" aria-hidden="true"/><input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Title, topic, or tag…" className="w-full rounded border border-border bg-card py-3 pl-10 pr-3 text-sm focus:outline-gold"/></span></label>
      ...
      <div className="flex items-center justify-between gap-4 mb-4 min-h-8"><p role="status" className="text-sm text-muted-foreground">{filtered.length} {filtered.length === 1 ? "article" : "articles"}</p>{(category || query) && <button onClick={reset} className="text-sm text-navy underline underline-offset-4">Clear filters</button>}</div>
